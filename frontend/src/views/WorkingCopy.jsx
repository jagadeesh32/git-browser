...
const FileItem = ({ path, status, actionIcon: ActionIcon, onAction, actionTitle, isSelected, onSelect }) => {
    const getStatusColor = (s) => {
        if (s === 'modified') return 'text-yellow-600 dark:text-yellow-400';
        if (s === 'added' || s === 'untracked') return 'text-green-600 dark:text-green-400';
        ...
        >
            ...
            <div className="opacity-0 group-hover:opacity-100 flex items-center">
                <button
                    ...
                    className="p-1 hover:bg-gray-300 dark:hover:bg-gray-600 rounded text-gray-500 dark:text-gray-300 transition-opacity"
                    title={actionTitle}
                >
                    <ActionIcon />
                </button>
...
const WorkingCopy = () => {
    ...
    return (
        <div className="flex h-full bg-white dark:bg-[#1e1e1e] transition-colors duration-200">
            ...
            <div className="w-96 flex flex-col border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#1e1e1e] transition-colors duration-200">

                {/* Actions Bar */}
                <div className="p-2 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-gray-100 dark:bg-[#252526] transition-colors duration-200">
                    <div className="flex space-x-1">
                        <ActionButton icon={VscRefresh} onClick={fetchStatus} title="Refresh" />
                        <ActionButton icon={VscSync} onClick={() => { }} title="Fetch" />
                        <ActionButton icon={VscCloudDownload} onClick={() => { }} title="Pull" />
                        <ActionButton icon={VscCloudUpload} onClick={() => { }} title="Push" />
                    </div>
                </div>
            ...
            <div className="flex-1 bg-white dark:bg-[#121212] flex flex-col overflow-hidden transition-colors duration-200 border-l border-gray-200 dark:border-gray-800">
                {selectedFile ? (
                    <>
