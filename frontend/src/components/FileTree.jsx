      ...
      <div>
        ...
        <div
          className={`
            ...
            hover:bg-gray-100 dark:hover:bg-[#2a2d2e] transition-colors
          `}
  ...
  return (
    <div
      className={`
        ...
        transition-colors
        ${isSelected
          ? 'bg-blue-50 dark:bg-[#37373d] text-blue-700 dark:text-white border-l-2 border-blue-500'
          : 'hover:bg-gray-100 dark:hover:bg-[#2a2d2e] text-gray-600 dark:text-gray-400 border-l-2 border-transparent'
        }
      `}
    ...
    >
      <div className="flex items-center gap-2 flex-1 min-w-0">
        <div className={isSelected ? 'text-blue-600 dark:text-white' : 'text-gray-500'}>
          {getFileIcon(name)}
        </div>
      ...
      <div className="flex-shrink-0 flex items-center gap-1 text-xs font-mono">
        {file.additions > 0 && (
          <span className={isSelected ? 'text-green-200' : 'text-green-400'}>
            +{file.additions}
          </span>
        )}
        {file.additions > 0 && file.deletions > 0 && (
          <span className={isSelected ? 'text-gray-400' : 'text-gray-600'}>
            /
          </span>
        )}
        {file.deletions > 0 && (
    ...
    <div className="py-2">
      ...
      <div className="px-2 mb-3">
        <div className="flex items-center justify-between">
          <label htmlFor="file-sort" className="block text-xs font-bold text-gray-500 uppercase tracking-wider">
            Sort by
          </label>
          <select
            id="file-sort"
            value={sortMode}
            onChange={(e) => setSetSortMode(e.target.value)}
            className="bg-gray-100 dark:bg-[#252526] text-gray-700 dark:text-gray-300 text-xs border-none rounded px-2 py-1 focus:outline-none cursor-pointer hover:bg-gray-200 dark:hover:bg-[#2a2d2e]"
          >
